...
function resolveSkillPath(skillName, superpowersDir, personalDir) {
    const forceSuperpowers = skillName.startsWith('ottopowers-gh:');
    const actualSkillName = forceSuperpowers ? skillName.replace(/^ottopowers-gh:/, '') : skillName;
...
// Test 2: ottopowers-gh: prefix should force superpowers
const forced = resolveSkillPath('ottopowers-gh:shared-skill', superpowersDir, personalDir);
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"superpowers"'; then
    echo "  [PASS] ottopowers-gh: prefix forces superpowers resolution"
else
    echo "  [FAIL] ottopowers-gh: prefix not working"
    exit 1
fi
